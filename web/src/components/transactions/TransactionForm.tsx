  ...
  return (
    <motion.form variants={itemVariants} onSubmit={handleSubmit} className="bg-white p-5 rounded-2xl shadow-sm border border-sky-100">
      <div className="flex bg-gray-100 p-1 rounded-xl mb-5">
        <button type="button" onClick={() => { setTab('expense'); form.setCategoryId(""); form.setCreditCardId(""); }} className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === 'expense' ? 'bg-white shadow-sm text-orange-600' : 'text-gray-500'}`}>Gasto</button>
        <button type="button" onClick={() => { setTab('income'); form.setCategoryId(""); form.setCreditCardId(""); }} className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === 'income' ? 'bg-white shadow-sm text-sky-600' : 'text-gray-500'}`}>Ingreso</button>
      </div>
        ...
        <AnimatePresence>
          {tab === 'expense' && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              ...
              <select value={form.creditCardId} onChange={(e) => form.setCreditCardId(e.target.value)} className="w-full px-3 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 font-medium">
...
export function EditTransactionForm({ transaction, categories, cards, onSave, onCancel }: EditTransactionFormProps) {
  ...
  const submitEdit = async (e: React.FormEvent) => {
    ...
    await onSave(transaction.id, {
      ...
      categoryId,
      note: note || undefined,
      creditCardId: isExpense && creditCardId ? creditCardId : null, // Manda el null como corresponde
    });
    setIsSaving(false);
  };
  ...
  return (
    <form onSubmit={submitEdit} className="space-y-4">
      ...
      <div className="mt-6 flex gap-3 pt-2 border-t border-gray-100">
        <button type="button" onClick={onCancel} className="flex-1 py-3 font-bold text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200">
          Cancelar
        </button>
