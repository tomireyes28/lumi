"use client"; // Clave agregar esto porque ahora usamos useState
...
import { RefundCalculator } from "@/components/calculator/RefundCalculator"; 
...
export function DashboardHeader({ summary, reminders, itemVariants }: DashboardHeaderProps) {
  ...
  // 3. Estado para controlar el modal de la calculadora
  ...
  return (
    <>
      <motion.header variants={itemVariants} className="mb-4 flex flex-col gap-6">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-2">
            {/* NUEVO: Botón Calculadora */}
            <button 
              onClick={() => setIsCalcOpen(true)}
              className="p-2.5 bg-gray-100/80 rounded-full text-gray-600 hover:bg-sky-100 hover:text-sky-600 transition-colors"
              title="Simulador de Reintegros"
            >
      ...
      {/* NUEVO: Modal de la Calculadora de Reintegros */}
      <Modal isOpen={isCalcOpen} onClose={() => setIsCalcOpen(false)} title="Simulador de Promos">
        <RefundCalculator />
      </Modal>
    </>
  );
